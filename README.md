...
- **Free Colab GPU = 15GB T4 VRAM.** Only `Wan 2.2 TextImage2Video 5B FastWan`
  at 480p fits reliably. A real 5-minute video means ~40-60 short clips
  (5-8 sec each) — on a free GPU that can take a very long time and Colab
  *will* disconnect idle/long sessions. Start by testing with `max_scenes`
  set low (5-10 scenes = ~40-60 sec of video) before trying to push to 5
  minutes.
