"""
Custom chat-style UI for Wan2GP (ChatGPT-like layout):
  - left drawer: configuration (model/resolution)
  ...
  - on send: a minimal card appears showing a live timer while it generates,
    then the resulting single video clip

No voice, no lip-sync, no multi-scene splitting/stitching - one story +
one character photo produces one Wan2GP video clip.
...
def progress_card(step_label: str, elapsed: float) -> str:
    return (
        '<div class="msg assistant"><div class="card">'
        '<div class="card-header">'
        '<span class="scene-label">Generating</span>'
...
def result_card(video_path: str) -> str:
    video_tag = ""
    if video_path and os.path.exists(video_path):
        with open(video_path, "rb") as f:
            b64 = base64.b64encode(f.read()).decode()
        video_tag = f'<video class="result-video" controls src="data:video/mp4;base64,{b64}"></video>'
    return (
        '<div class="msg assistant"><div class="card">'
...
def run_pipeline(chat_html, character_image, story_text, model_label):
    if not character_image:
        chat_html += error_card("Please attach a character reference photo first.")
        yield chat_html, chat_html, ""
        return
    if not story_text or not story_text.strip():
        chat_html += error_card("Please type your story/prompt first.")
        yield chat_html, chat_html, ""
        return
    ...
    out_path = os.path.join(run_dir, "clip.mp4")

    base_html = chat_html
    executor = concurrent.futures.ThreadPoolExecutor(max_workers=1)
    future = executor.submit(generate_clip, story_text, character_image, out_path, resolution)
    step_start = time.time()

    try:
        while not future.done():
            elapsed = time.time() - step_start
            chat_html = base_html + progress_card("Generating video clip...", elapsed)
            yield chat_html, chat_html, ""
            time.sleep(1)
        result_path = future.result()
    except Exception as e:
        chat_html = base_html + error_card(f"Generation failed: {e}")
        yield chat_html, chat_html, ""
        return

    chat_html = base_html + result_card(result_path)
    yield chat_html, chat_html, ""
...
.progress-fill.indeterminate {{
    height: 3px; width: 40%; background: var(--text); border-radius: 2px;
...
def build_app() -> gr.Blocks:
    with gr.Blocks(css=CUSTOM_CSS, title="Video Generator") as demo:
        with gr.Column(elem_id="app-shell"):
                ...
                with gr.Column(scale=0, min_width=210, visible=False) as drawer:
                    with gr.Column(elem_id="drawer"):
                        ...
                        gr.HTML(
                            ...
                            'Attach a character photo, describe the scene, and pick a '
                            'model quality from the dropdown next to the input box.</div>'
                        )
            ...
            with gr.Row(elem_id="input-bar"):
                ...
                story_text = gr.Textbox(
                    placeholder="Describe your scene...", show_label=False, container=False,
        ...
        def wrapped_pipeline(*args):
            for chat_html, state_html, cleared_text in run_pipeline(*args):
                yield render(chat_html), state_html, cleared_text

        send_btn.click(
            fn=wrapped_pipeline,
            inputs=[chat_state, character_image, story_text, model],
